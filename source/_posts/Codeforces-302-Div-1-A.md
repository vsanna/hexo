...
\end{aligned}
$$
（tは$0 \leq j-t; 0 \leq k-t \cdot a[i-1]$を満たす間回す）
（左に詰めて欲しい）
...
おや、どこかで見たことがある……。
dp[i][j][k]の式の2行目以降と一致してるようだ。
dp[i][j][k]を求める時にはdp[i][j-1][k]は既に計算されている（dp表をi, j, kの三重ループで0から埋める）ため、この値が利用できる。
...
再度漸化式を凝視すると、dp[i]はdp[i-1]のみに依存しているので、iの所は大きさ2の配列を交互に使えば十分。
これで、2MBになりメモリもOK。
